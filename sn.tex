...
We study below the impact of cadence on the size and quality of the
type Ia supernova sampled. In the course of this study, we were driven
by two main considerations.

First, SN cosmology in LSST is now systematics-limited.  Increasing
the statistics in the Hubble diagram must be coupled with (1) advances
in the measurement of the SN distances (i.e. a control at the per-mil
level of the photometry and survey flux calibration, and possibly a
3-parameter SN standardization technique) (2) a better control of the
SN astrophysical environment and its potential impacts on the SN
...
Second, access to spectroscopy will not scale with the large amount of
SNe LSST will deliver.  Only $\sim$ 10\% of LSST SNe will benefit from
a live spectrum, and obtaining a host redshift for the full LSST
sample using the fiber spectrographs available in the southern
hemisphere is doable, but challenging.  The main consequence of this
situation is that all the studies listed above, in particular
identification and standardization will rely primarily on the
supernova light curves.  Obtaining high quality SN light curves is
therefore the key design point of the SN survey.

Also, since spectroscopic time is scarse, we cannot afford to waste
it.  This means that all transients identified spectroscopically,
around peak luminosity, as SNe~Ia, {\em must} eventually have light
curves of sufficient quality to end up in the Hubble diagram.  This
puts a strong requirement on the regularity and predictability of the
observing strategy.

Since light curve sampling quality is at the core of the design of the
LSST SN survey, we propose to adopt as our main metric, the size and
redshift extent of the subset of Hubble-diagram-grade supernovae. By
Hubble-diagram-grade supernova, we mean a supernova, with a light
curve sampling which is good enough to allow measuring a standardized
distance with a measurement uncertainty lower than the SN-to-SN
...
Three classes of observing strategies have been studied in detail:
\begin{itemize}
